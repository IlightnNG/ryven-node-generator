...
import project_workspace as project_ws
...
import generator
from node_preview import NodePreviewWidget
...
class _AITurnWorker(QThread):
    ...
    def run(self):
        try:
            from ai_assistant.config import ai_stream_enabled
            from ai_assistant.service import run_assistant_turn, run_turn_respecting_stream_flag

            if ai_stream_enabled():
                r = run_turn_respecting_stream_flag(
...
class PortCard(QFrame):
    ...
    def __init__(self, is_input=True, data=None):
        ...
        if is_input:
            ...
            if data and "widget" in data:
                self.w_type.setCurrentText(data["widget"].get("type", "None"))
                self.w_pos.setCurrentText(data["widget"].get("pos", "besides"))
                self.w_args.setText(data["widget"].get("args", ""))
...
class GeneratorDesignerUI(QMainWindow):
    ...
    def _build_preview_panel(self):
        ...
        ai_header = QLabel(
            "Assistant · API keys in <code>Generator/.env</code> "
    ...
    def _update_json_preview(self):
        if (
            ...
            from ai_assistant.preview_diff import json_list_diff_html
    ...
    def _ai_try_present_preview(self, result: dict):
        from ai_assistant.merge import apply_config_patch
        from ai_assistant.preview_diff import node_changed_keys
